...
6. 使用中文优先的 Conventional Commit，在本地形成待验收版本。
7. 向用户提交人工验收清单并停止；未经用户明确授权，不得推送分支或创建 PR。
8. 用户确认当前待验收版本后，推送当前分支并创建一个关联该 Issue 的 PR，等待 CI 和审查。
9. PR 合并并关闭 Issue 后，删除分支，再开始下一项。

## 人工验收门禁

本地实现完成不等于允许推送。请求用户验收时必须提供：

- Issue 编号与链接；
- 当前 `nora/` 分支和本地 Commit；
- 基于真实 diff 的变更摘要；
- 已执行验证及其实际结果；
- 未执行检查及原因；
- 用户可以直接操作的验收步骤。

用户要求修改时，继续在同一 Issue 和分支完成本地修正、验证和 Commit，然后重新提交验收。用户的推送授权只对当次展示的
待验收版本有效；授权后若又发生实质修改，必须重新请求验收。只有用户明确表达“可以推送”“推送吧”等同等含义后，才能
执行 `git push` 和创建 PR。该授权不包含合并授权，PR 仍须通过 CI、审查和单独的显式合并门禁。
...
创建时正文状态只使用 `ready` 或 `blocked`；创建分支并产生实质修改后更新为 `in-progress`；本地待验收版本准备完成后更新为
`acceptance`；创建 PR 后更新为 `review`。用户要求修改时从 `acceptance` 返回 `in-progress`。不创建重复的 `status:*`
标签。Parent Epic 只表示层级归属，真正阻塞执行的内容单独写入“依赖”。
