...
> 状态：Architecture Issue #98 的执行基线；文件名暂保留 `DRAFT`，以兼容已建立的 GitHub 链接。
> 基线日期：2026-08-02。
...
当前 JobPosting 领域对象具有 `job_title`、`company_name`、`location`，但：

- 创建请求没有公开这些字段；
- 创建/读取响应不返回 JD 正文、标题、公司、地点、来源和版本；
- 没有用户范围的岗位列表接口；
- 前端刷新后无法恢复用户最近岗位；
- M3 的结构化规则不能仅依赖摘要执行。

这些缺口进入 M2，而不是回写 M1 历史范围。
