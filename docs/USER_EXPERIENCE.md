...
用户可以提交 JD 文本、链接或截图。Current 路径只把链接抓取或 OCR 正文回填为可编辑预览；D-021 目标流程会继续清洗正文，
用 AI 自动填充职位、公司、地点和结构化岗位要求。用户可修改任意字段并一次整体确认，确认时原子创建 `JobPosting` 与首个
`JobRequirementSnapshot`；确认前不写岗位事实。页面显示来源、时间、AI 候选和未知项；公司公开信息研究在后续阶段保留
来源、许可范围和可信等级。
...
| OCR 与受控链接输入 | API Current，浏览器入口 Planned | API 返回候选预览；浏览器三模式入口尚未接线 |
| PDF/DOCX 简历 AI 导入 | Planned，M5 / D-021 | 文本提取或扫描 PDF OCR 后生成可编辑主档草稿；一次整体确认，不读取项目链接，不自动发布简历版本 |
| JD AI 自动填充 | Planned，M5 / D-021 | 文本/截图/受控链接统一生成职位、公司、地点和要求草稿；一次整体确认后原子创建岗位及首个要求快照 |
