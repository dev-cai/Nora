...
岗位信息优先从文本、截图或受控链接提取并生成待确认的结构化候选；AI 不可用时，用户才可用完整手工字段作为最后兜底。
