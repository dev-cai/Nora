...
| 状态 | 含义 | 当前范围 |
| :--- | :--- | :--- |
| **Current** | 已实现并有验证证据 | 仓库治理、M0/M1、岗位/主档/简历、结构化岗位要求（JobRequirementSnapshot）、受控链接抓取与截图 OCR 输入、Vue 工作台、JD 输入契约，以及 M2 输入、M3 确定性决策和 M4 投递闭环；公司情报、材料生成、手工投递/面试记录、恢复与隔离已有完整浏览器门禁，并已交付 fail-closed 的 Beta 自动发布/回滚控制面，逐项范围、代码路径和证据只见能力台账 |
