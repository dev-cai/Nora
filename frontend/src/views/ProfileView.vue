...
async function save(): Promise<void> {
  ...
  saving.value = true
  try {
    let payload = cloneProfileInput(draft.value)
    ...
    if (importedDraft.value) payload = confirmImportedProfile(payload)
...
async function importResumePdf(event: Event): Promise<void> {
  ...
  try {
    ...
    importedDraft.value = true
    success.value = "AI 已解析简历，请检查候选字段后一次确认导入主档"
  } catch (reason) { error.value = userMessage(reason) }
  ...
  <AppShell>
    <section class="section-toolbar page-toolbar">
      <div>
        <p class="eyebrow">
          候选人事实源
        </p><h2>我的主档</h2><p>{{ importedDraft ? "检查 AI 候选字段后一次确认导入。" : "维护可追溯的事实，并明确每个字段的确认状态。" }}</p>
      </div>
    ...
    >
        ...
        <div class="fact-grid two-columns">
          <div class="fact-field">
            <label>姓名<input
              ...
              maxlength="200"
            ></label><ConfirmationSelect v-if="!importedDraft" v-model="draft.basic_information.display_name.confirmation_status" />
          </div>
          <div class="fact-field">
            <label>当前所在地<input
              v-model="draft.basic_information.current_location.value"
              maxlength="200"
            ></label><ConfirmationSelect v-if="!importedDraft" v-model="draft.basic_information.current_location.confirmation_status" />
          </div>
        </div>
      </section>
      ...
      <section class="form-section">
        ...
        <div class="fact-grid two-columns">
          <div class="fact-field">
            ...
            /></label><ConfirmationSelect v-if="!importedDraft" v-model="draft.preferences.target_locations.confirmation_status" />
          </div>
          <div class="fact-field">
            <label>目标职位<textarea
            ...
            /></label><ConfirmationSelect v-if="!importedDraft" v-model="draft.preferences.target_roles.confirmation_status" />
          </div>
          <div class="fact-field compact">
            <label class="checkbox-label"><input
              v-model="draft.preferences.accepts_remote.value"
              type="checkbox"
            >接受远程工作</label><ConfirmationSelect v-if="!importedDraft" v-model="draft.preferences.accepts_remote.confirmation_status" />
          </div>
        </div>
      </section>
          ...
          <div class="fact-grid three-columns">
            <div class="fact-field">
              <label>学校<input
                ...
                maxlength="200"
              ></label><ConfirmationSelect v-if="!importedDraft" v-model="item.school.confirmation_status" />
            </div>
            <div class="fact-field">
              <label>学历<input
                v-model="item.degree.value"
                maxlength="200"
              ></label><ConfirmationSelect v-if="!importedDraft" v-model="item.degree.confirmation_status" />
            </div>
            <div class="fact-field">
              <label>专业<input
                v-model="item.major.value"
                maxlength="200"
              ></label><ConfirmationSelect v-if="!importedDraft" v-model="item.major.confirmation_status" />
            </div>
            <div class="fact-field">
              <label>开始日期<input
                v-model="item.start_date.value"
                type="date"
              ></label><ConfirmationSelect v-if="!importedDraft" v-model="item.start_date.confirmation_status" />
            </div>
            <div class="fact-field">
              <label>结束日期<input
                v-model="item.end_date.value"
                type="date"
              ></label><ConfirmationSelect v-if="!importedDraft" v-model="item.end_date.confirmation_status" />
            </div>
          </div>
        </div>
      </section>
          ...
          <div class="fact-grid two-columns">
            <div class="fact-field">
              <label>公司<input
                ...
                maxlength="200"
              ></label><ConfirmationSelect v-if="!importedDraft" v-model="item.company.confirmation_status" />
            </div>
            <div class="fact-field">
              <label>职位<input
                v-model="item.job_title.value"
                maxlength="200"
              ></label><ConfirmationSelect v-if="!importedDraft" v-model="item.job_title.confirmation_status" />
            </div>
            <div class="fact-field">
              <label>开始日期<input
                v-model="item.start_date.value"
            ...
            <div class="fact-field">
              ...
              /></label><ConfirmationSelect v-if="!importedDraft" v-model="item.responsibilities.confirmation_status" />
            </div>
            <div class="fact-field">
              ...
              /></label><ConfirmationSelect v-if="!importedDraft" v-model="item.achievements.confirmation_status" />
            </div>
          </div>
        </div>
      </section>
          ...
          <div class="fact-grid three-columns">
            <div class="fact-field">
              <label>技能名称<input
                ...
                maxlength="200"
              ></label><ConfirmationSelect v-if="!importedDraft" v-model="item.name.confirmation_status" />
            </div>
            <div class="fact-field">
              <label>熟练度<input
                v-model="item.proficiency.value"
                maxlength="200"
              ></label><ConfirmationSelect v-if="!importedDraft" v-model="item.proficiency.confirmation_status" />
            </div>
            <div class="fact-field">
              <label>使用年限<input
                ...
                max="100"
                step="0.5"
              ></label><ConfirmationSelect v-if="!importedDraft" v-model="item.years.confirmation_status" />
            </div>
          </div>
          <button
      ...
      <div class="sticky-actions">
        ...
        >
          <Save :size="17" />{{ saving ? "正在保存…" : importedDraft ? "确认导入主档" : "保存主档新版本" }}
        </button>
      </div>
    </form>
  </AppShell>
