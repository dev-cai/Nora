...
def upgrade() -> None:
    op.create_table(
        ...
        sa.CheckConstraint("interview_case_version >= 1", name="ck_interview_preparation_case_version"),
        sa.ForeignKeyConstraint(["owner_id"], ["users.id"], ondelete="CASCADE"),
        sa.ForeignKeyConstraint(["interview_case_id", "interview_case_version", "owner_id"], ["interview_cases.id", "interview_cases.version", "interview_cases.owner_id"], name="fk_interview_preparation_case_owner", ondelete="CASCADE"),
        sa.PrimaryKeyConstraint("id"),
        sa.UniqueConstraint("owner_id", "interview_case_id", "version", name="uq_interview_preparation_version"),
    )
    op.create_index("ix_interview_preparations_owner_id", "interview_preparations", ["owner_id"])
    op.create_index("ix_interview_preparations_interview_case_id", "interview_preparations", ["interview_case_id"])


def downgrade() -> None:
    op.drop_index("ix_interview_preparations_interview_case_id", table_name="interview_preparations")
